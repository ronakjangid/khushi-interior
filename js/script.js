...
// Wait until all images/videos are fully loaded
window.addEventListener('load', function () {
  ...
  if (total === 0) finishLoading(); // no assets to wait for
  ...
  mediaElements.forEach(el => {
    if (el.complete || (el.readyState >= 3)) {
      checkDone(); // already loaded
    } else {
      if (el.tagName.toLowerCase() === 'img') {
        el.addEventListener('load', checkDone);
        el.addEventListener('error', checkDone); // in case of broken links
      } else if (el.tagName.toLowerCase() === 'video') {
        el.addEventListener('loadeddata', checkDone);
        el.addEventListener('error', checkDone);
      }
    }
  });
